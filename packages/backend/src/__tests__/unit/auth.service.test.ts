...
describe('AuthService', () => {
  ...
  describe('login', () => {
    it('should successfully login with valid credentials', async () => {
      ...
      const decodedToken = jwt.verify(result.token, process.env.JWT_SECRET || 'test-secret') as any;
    ...
    it('should throw error for incorrect password', async () => {
      ...
      await expect(authService.login('service@test.com', 'wrongpassword')).rejects.toThrow(
        'Invalid email or password'
      );
    });
    ...
    it('should throw error for inactive user', async () => {
      ...
      await expect(authService.login('inactive@test.com', 'password123')).rejects.toThrow(
        'Invalid email or password'
      );
    });
  });
  ...
  describe('getUserProfile', () => {
    ...
    it('should throw error for non-existent user ID', async () => {
      await expect(authService.getUserProfile('507f1f77bcf86cd799439011')).rejects.toThrow(
        'User not found'
      );
    });
