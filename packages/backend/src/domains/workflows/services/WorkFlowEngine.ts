...
import { IWorkflow, IWorkflowNode, IWorkflowEdge } from '../../../models/Workflow.js';
import { IExecution, INodeExecution, Execution } from '../../../models/Execution.js';
import { Credential } from '../../../models/Credentials.js';
import { logger } from '../../../utils/logger.js';
import GmailService, { SendEmailOptions } from '../../oauth/services/GmailService.js';
...
export class WorkflowEngine extends EventEmitter {
  ...
  private async executeNodes(
    ...
    while (executeQueue.length > 0) {
      ...
      if (currentNode.type === 'condition' && result.output?.outputPath) {
        ...
        if (matchingEdge) {
          ...
          if (nextNode && !executedNodes.has(nextNode.id)) {
            logger.info(`Condition routing: ${currentNode.id} -> ${nextNode.id} via ${result.output.outputPath}`);
      ...
      this.emit('execution:progress', {
        ...
        progress: (executedNodes.size / workflow.nodes.length) * 100,
      });
    }
  }
  ...
  private async loadCredentials(userId: string): Promise<Record<string, any>> {
    const credentials = await Credential.find({ userId, isActive: true }).select('+data');
  ...
  private async executeCondition(
    ...
    const conditionRules = node.data.configuration?.conditionRules || 
                          node.data.conditionRules || 
                          [];
    
    const defaultOutput = node.data.configuration?.defaultOutput || 
  ...
  private evaluateCondition(fieldValue: any, operator: string, compareValue: any): boolean {
    try {
      ...
      logger.warn('Error evaluating condition:', { fieldValue, operator, compareValue, error });
      return false;
    }
  }
