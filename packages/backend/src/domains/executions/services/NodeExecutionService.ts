...
import { type GmailCredentials, GmailService } from '../../oauth/services/GmailService.js';
...
export class NodeExecutionService {
  ...
  private async executeConditionNode(node: any, inputData: any): Promise<any> {
    ...
    const conditionRules =
      nodeConfig.configuration?.conditionRules || nodeConfig.conditionRules || [];

    const defaultOutput =
      nodeConfig.configuration?.defaultOutput || nodeConfig.defaultOutput || 'default';
  ...
  private async executeGmailSendNode(node: any, inputData: any, userId: string): Promise<any> {
    try {
      ...
      // Ensure we have a string
      if (typeof credentialId !== 'string') {
        throw new Error('Invalid Gmail Send credential ID format - must be a string');
      }
      ...
      try {
        ...
        throw new Error(`Failed to decrypt Gmail Send credentials: ${error.message}`);
      }
      ...
      const emailOptions: SendEmailOptions = {
        ...
        body: nodeConfig.message || inputData?.message || inputData?.responseText || '',
