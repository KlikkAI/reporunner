...
const ConditionBuilder: React.FC<ConditionBuilderProps> = ({
  ...
  const testRule = (rule: ConditionRule) => {
    ...
    // We just need to trigger the onTestRule function.
    onTestRule(rule.id, {})
  }
  ...
  const tryParseJsonString = (str: string): any => {
    ...
    if (cleanStr.includes('```json')) {
      const jsonPattern = new RegExp('```json\\s*([\\s\\S]*?)\\s*```')
      const match = cleanStr.match(jsonPattern)
      if (match) {
    ...
    if (
      ...
      try {
        ...
        jsonParseCache.set(str, parsed) // Cache the result
        return parsed
      } catch (error) {
        jsonParseCache.set(str, null) // Cache the failure
        return null
      }
    }
  ...
  const getFieldValue = (data: any, fieldPath: string) => {
    try {
      // Handle simple field names and dot notation paths
      ...
      for (let i = 0; i < pathParts.length; i++) {
        ...
        // Handle array index notation (e.g., "items.0" or "emails[1]")
        if (key.includes('[') && key.includes(']')) {
          const bracketPattern = new RegExp('[\\[\\]]')
          const [arrayKey, indexStr] = key.split(bracketPattern).filter(Boolean)
        ...
        if (typeof currentValue === 'string' && i < pathParts.length - 1) {
          const parsed = tryParseJsonString(currentValue)
          if (parsed !== null) {
            currentValue = parsed
...
const ConditionRuleCard: React.FC<{
  ...
  canDelete: boolean
  availableOutputs?: Array<{ id: string; label: string }>
}> = ({
  rule,
  index,
  availableFields,
