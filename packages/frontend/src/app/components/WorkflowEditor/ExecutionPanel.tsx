...
export const ExecutionPanel: React.FC<ExecutionPanelProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "console" | "variables" | "metrics" | "history"
  >("console");
  ...
  const getExecutionVariables = () => {
    ...
    nodeStates.forEach((state, nodeId) => {
      ...
      if (state.debugInfo?.watchedVariables) {
        Object.entries(state.debugInfo.watchedVariables).forEach(
          ([key, value]) => {
  ...
  const renderProgressBar = () => {
    ...
    const { progressPercentage, completedNodes, totalNodes, failedNodes } =
      progress;
          ...
          <div className="bg-gray-900 rounded border border-gray-700 p-3">
            ...
            {performanceMetrics.nodeExecutionTimes.size === 0 ? (
              ...
              <div className="space-y-1">
                {Array.from(
                  performanceMetrics.nodeExecutionTimes.entries(),
                ).map(([nodeId, time]) => (
  ...
  return (
    ...
    >
      ...
      <div className="flex border-b border-gray-700">
        {(["console", "variables", "metrics", "history"] as const).map(
          (tab) => (
            <button
              key={tab}
