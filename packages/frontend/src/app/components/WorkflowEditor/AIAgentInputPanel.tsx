      ...
      <div className="h-full flex items-center justify-center text-gray-400">
        <div className="text-center space-y-3">
          ...
          <div className="text-xs text-gray-500 max-w-48">
            Connect nodes like Gmail Trigger or Transform to provide input data
            for AI processing
          </div>
      ...
      <div className="mt-6 p-3 bg-gray-800 rounded-lg border border-gray-600">
        ...
        <div className="space-y-2 text-xs text-gray-400">
          ...
          <div className="flex items-start space-x-2">
            <span className="text-orange-400 mt-0.5">•</span>
            <span>
              The AI will automatically detect and process structured data from
              connected nodes
            </span>
          </div>
        </div>
...
const InputDataRenderer: React.FC<{ data: any; nodeType: string }> = ({
  ...
  if (nodeType === 'transform') {
    return (
      <div className="space-y-2">
        <div className="text-xs text-gray-400 mb-2">
          Transformed data ready for AI processing
        </div>
        ...
        {fields.slice(0, 5).map(field => {
          ...
          return (
            <div key={field} className="flex items-start space-x-2 text-sm">
              <span className="text-gray-400 w-20 flex-shrink-0 capitalize">
                {field}:
              </span>
