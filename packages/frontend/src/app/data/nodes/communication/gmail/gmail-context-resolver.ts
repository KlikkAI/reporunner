...
import type { ContextResolver, WorkflowContext, ResolvedContext } from '../../../../core/nodes/registry'
...
export const gmailContextResolver: ContextResolver = {
  ...
  async resolve(context: WorkflowContext): Promise<ResolvedContext | null> {
    ...
    const properties = await this.resolveProperties(context, mode, resource, operation)
   ...
   */
  detectGmailMode(context: WorkflowContext): 'trigger' | 'action' | 'webhook' | 'poll' {
    ...
    if (explicitMode && ['trigger', 'action', 'webhook', 'poll'].includes(explicitMode)) {
   ...
   */
  detectResource(context: WorkflowContext, mode: 'trigger' | 'action' | 'webhook' | 'poll'): string {
    // Check explicit resource selection
    const explicitResource = context.formState?.resource as string
    if (explicitResource && ['email', 'label', 'draft', 'thread'].includes(explicitResource)) {
...
export function registerGmailContextResolver() {
  ...
  import('@/core/nodes/registry').then(({ nodeRegistry }) => {
