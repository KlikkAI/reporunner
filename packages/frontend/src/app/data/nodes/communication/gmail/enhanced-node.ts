...
export const gmailEnhancedNode: EnhancedIntegrationNodeType = {
  ...
  description: 'Unified Gmail integration with smart mode detection - automatically adapts for trigger or action use based on workflow context',
    ...
    groups: [
      {
        ...
        properties: ['credential', 'resource', 'operation'],
        collapsible: false,
        collapsed: false
      },
      {
        ...
        properties: ['event', 'pollTimes', 'filters'],
        collapsible: true,
        collapsed: false
      },
      {
        ...
        properties: ['sendTo', 'sendCc', 'sendBcc', 'subject', 'message', 'emailType'],
        collapsible: true,
        collapsed: false
      },
      {
        ...
        properties: ['simplify', 'downloadAttachments', 'maxResults', 'attachments'],
        collapsible: true,
        collapsed: true
      }
    ]
...
export const gmailNodeCapabilities = {
  ...
  operations: {
    email: {
      ...
      action: ['send', 'reply', 'forward', 'get', 'getAll', 'delete', 'markAsRead', 'markAsUnread', 'addLabels', 'removeLabels']
    },
    label: {
      trigger: [],
...
export async function executeGmailEnhanced(
  ...
  switch (resource) {
    case 'email':
      return executeEmailOperations(operation, nodeParameters, credentials, context)
    case 'label':
...
async function executeEmailReply(parameters: any, credentials: any): Promise<any[]> {
  console.log('↩️ Mock: Email reply operation')
  return [{ operation: 'reply', status: 'mock_implementation' }]
}

async function executeEmailForward(parameters: any, credentials: any): Promise<any[]> {
  console.log('➡️ Mock: Email forward operation')
  return [{ operation: 'forward', status: 'mock_implementation' }]
}

async function executeEmailGet(parameters: any, credentials: any): Promise<any[]> {
  console.log('📧 Mock: Email get operation')
  return [{ operation: 'get', status: 'mock_implementation' }]
}

async function executeEmailGetAll(parameters: any, credentials: any): Promise<any[]> {
  console.log('📬 Mock: Email getAll operation')
  return [{ operation: 'getAll', status: 'mock_implementation' }]
}

async function executeEmailDelete(parameters: any, credentials: any): Promise<any[]> {
  console.log('🗑️ Mock: Email delete operation')
  return [{ operation: 'delete', status: 'mock_implementation' }]
}

async function executeEmailMarkRead(operation: string, parameters: any, credentials: any): Promise<any[]> {
  console.log(`👁️ Mock: Email ${operation} operation`)
  return [{ operation, status: 'mock_implementation' }]
}

async function executeEmailLabelManagement(operation: string, parameters: any, credentials: any): Promise<any[]> {
  console.log(`🏷️ Mock: Email ${operation} operation`)
  return [{ operation, status: 'mock_implementation' }]
}
