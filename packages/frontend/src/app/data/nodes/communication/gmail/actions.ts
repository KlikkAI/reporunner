...
export async function executeGmailTrigger(
  ...
  try {
    ...
    console.log(
      "Number of emails:",
      Array.isArray(emailData) ? emailData.length : "not an array",
    );
    ...
    throw new Error(
      `Failed to fetch Gmail emails: ${error.message}. Please check your Gmail credentials and connection.`,
    );
  }
}
...
export async function createDraftTool(
  ...
  console.log("AI Tool: createDraft called with:", {
    subject,
    message,
    threadId,
    sendTo,
...
export async function executeGmailArchive(
  ...
  const messageIds = Array.isArray(parameters.messageId)
...
export async function executeGmailSmartReplies(
  ...
  console.log(
    "Executing Gmail smart replies generation with parameters:",
    parameters,
  );
