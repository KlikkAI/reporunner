...
export async function demoGmailEnterpriseNode() {
  ...
  console.log('- Features:', Object.keys(gmailEnhanced.meta.features).filter(f => gmailEnhanced.meta.features[f]))
  ...
  try {
    const triggerProperties = await nodeRegistry.resolveNodeProperties('gmail-enhanced', triggerContext)
    console.log('- Trigger Properties Count:', triggerProperties.length)
    
    const actionProperties = await nodeRegistry.resolveNodeProperties('gmail-enhanced', actionContext)
  ...
  try {
    ...
    if (enhancedNode) {
      ...
      console.log('- Configuration Properties:', enhancedNode.configuration?.properties?.length || 0)
