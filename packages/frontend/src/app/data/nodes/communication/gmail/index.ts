...
import { gmailEnhancedNode, gmailNodeCapabilities, executeGmailEnhanced, testGmailEnhancedConnection } from './enhanced-node'
...
import { executeGmailTrigger, executeGmailSend, testGmailConnection } from './actions'
...
export function initializeGmailNode(): void {
  ...
  console.log('🔍 Enhanced nodes in registry:', enhancedNodes.map(node => ({ id: node.id, displayName: node.displayName })))
  ...
  console.log('📋 All registered node descriptions:', allDescriptions.map(n => ({
    name: n.name,
  ...
  console.log('🎯 Gmail node found in registry:', gmailNode ? {
    name: gmailNode.name,
