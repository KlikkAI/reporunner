...
export const gmailEnhancedCoreProperties: NodeProperty[] = [
  ...
  },
  {
    ...
    default: 'email',
    options: [
      { name: 'Email', value: 'email', description: 'Work with email messages' },
      { name: 'Label', value: 'label', description: 'Manage Gmail labels' },
      { name: 'Draft', value: 'draft', description: 'Create and manage drafts' },
      { name: 'Thread', value: 'thread', description: 'Work with email threads' }
    ],
    noDataExpression: true
  }
]
...
export const gmailEmailProperties: NodeProperty[] = [
  ...
  // Scheduled Send Time (High Priority - Very requested feature)
  {
    ...
    displayName: 'Send At',
    type: 'dateTime',
    description: 'Schedule email to be sent at specific time (leave empty to send immediately)',
    displayOptions: {
      show: {
        resource: ['email'],
        operation: ['send', 'reply', 'forward']
  ...
  // Attachment Type Filtering (Medium Priority - Useful for specific workflows)
  {
    ...
    options: [
      { name: 'Images (jpg, png, gif)', value: 'filename:(jpg OR png OR gif OR jpeg)' },
      { name: 'Documents (pdf, doc, docx)', value: 'filename:(pdf OR doc OR docx)' },
      { name: 'Spreadsheets (xls, xlsx, csv)', value: 'filename:(xls OR xlsx OR csv)' },
  ...
  // Reply/Forward specific properties
  {
    ...
    displayOptions: {
      show: {
        resource: ['email'],
        operation: ['reply', 'forward', 'get', 'delete', 'markAsRead', 'markAsUnread']
      }
    }
  },
...
export const gmailThreadProperties: NodeProperty[] = [
  {
    ...
    options: [
      ...
      { name: 'Add Labels', value: 'addLabels', action: 'Add labels to thread' },
      { name: 'Remove Labels', value: 'removeLabels', action: 'Remove labels from thread' }
    ]
  },
  {
    name: 'threadId',
...
export const gmailAdvancedProperties: NodeProperty[] = [
  ...
  },
  {
    ...
    type: 'boolean',
    description: 'Track clicks on links in emails (requires third-party service)',
    default: false,
    displayOptions: {
      show: {
        resource: ['email'],
  ...
  },
  {
    ...
    displayOptions: {
      ...
      hide: {
        autoReplyMessage: [null, undefined]
      }
    }
  },
  {
  ...
  // Conditional Sending Logic
  {
    ...
    type: 'string',
    description: 'JavaScript expression to determine if email should be sent (return true/false)',
