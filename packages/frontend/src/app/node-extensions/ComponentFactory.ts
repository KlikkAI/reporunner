...
class ComponentFactory implements UIComponentFactory {
  ...
  registerBodyComponent(name: string, component: ComponentType<CustomNodeBodyProps>): void {
    if (!component) {
      console.error(`❌ ComponentFactory: Attempted to register null/undefined body component: ${name}`)
      return
    }
    ...
    console.log(`✅ ComponentFactory: Body component "${name}" registration verified:`, !!retrieved)
  }

  registerPropertiesPanel(name: string, component: ComponentType<CustomPropertiesPanelProps>): void {
    if (!component) {
      console.error(`❌ ComponentFactory: Attempted to register null/undefined properties panel: ${name}`)
      return
    }
    ...
    console.log(`✅ ComponentFactory: Properties panel "${name}" registration verified:`, !!retrieved)
  }
  ...
  registerLazyPropertiesPanel(
    name: string,
    loader: () => Promise<{ default: ComponentType<CustomPropertiesPanelProps> }>
  ): void {
  ...
  async loadPropertiesPanel(name: string): Promise<ComponentType<CustomPropertiesPanelProps> | null> {
    ...
    if (this.loadingPromises.has(loaderKey)) {
      return (await this.loadingPromises.get(loaderKey)) as ComponentType<CustomPropertiesPanelProps>
    }
