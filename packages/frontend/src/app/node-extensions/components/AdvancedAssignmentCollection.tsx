...
import { TypeInferenceEngine, AdvancedTypeValidator, AssignmentValidator } from '../../../core/utils/typeValidation'
...
const AdvancedAssignmentItem: React.FC<{
  ...
  allowedTypes = ['stringValue', 'numberValue', 'booleanValue', 'arrayValue', 'objectValue', 'dateValue'],
    ...
    if (autoDetectTypes && assignment.value && assignment.value !== '') {
      ...
      if (inferredType !== assignment.type) {
        onUpdate(assignment.id, { 
          inferredType,
          type: inferredType // Auto-apply inferred type
        })
      }
    }
  ...
  useEffect(() => {
    if (enableValidation && assignment.name && assignment.value !== '') {
      ...
      const validator = new AssignmentValidator({ ignoreConversionErrors: true, strictMode: false })
  ...
  const handleFieldChange = useCallback((field: keyof IAdvancedAssignment, value: any) => {
  ...
  const handleTypeChange = useCallback((newType: string) => {
    ...
    try {
      const validator = new AdvancedTypeValidator({ ignoreConversionErrors: true, strictMode: false })
  ...
  const renderValueInput = () => {
    const commonProps = {
      ...
      onChange: (e: any) => handleFieldChange('value', typeof e === 'object' ? e.target?.value || e : e),
      disabled,
      ...
      case 'booleanValue':
        return (
          ...
          >
            ...
            {isExpression && <Option value={assignment.value}>Expression: {assignment.value}</Option>}
          </Select>
        )
    ...
    <>
      ...
      >
            ...
            <Col>
              <Space align="center">
                ...
                <Badge count={index + 1} size="small" style={{ backgroundColor: '#1890ff' }}>
                  <Text strong style={{ fontSize: '13px' }}>Assignment</Text>
                </Badge>
                {assignment.inferredType && assignment.inferredType !== assignment.type && (
                  <Tag color="blue" size="small">
                    Auto: {assignment.inferredType.replace('Value', '')}
                  </Tag>
                )}
                {isExpression && (
        ...
        <Space direction="vertical" style={{ width: '100%' }} size="small">
          ...
          <div>
            <Text type="secondary" style={{ fontSize: '11px', marginBottom: '4px', display: 'block' }}>
              Field Name
          ...
          {showTypeSelector && (
            <div>
              <Text type="secondary" style={{ fontSize: '11px', marginBottom: '4px', display: 'block' }}>
                Field Type
                {autoDetectTypes && (
                  <Tag size="small" style={{ marginLeft: '8px' }}>Auto-detect</Tag>
                )}
              </Text>
              <Select
            ...
            <Text type="secondary" style={{ fontSize: '11px', marginBottom: '4px', display: 'block' }}>
              ...
              {isExpression && (
                <Space style={{ marginLeft: '8px' }}>
                  <Tag size="small" color="purple">Expression Mode</Tag>
                </Space>
              )}
            </Text>
            {renderValueInput()}
          ...
          {assignment.validation && !assignment.validation.valid && (
            <Alert
              message={assignment.validation.error}
              type="error"
              size="small"
              showIcon
            />
                ...
                <Space direction="vertical" style={{ width: '100%' }} size="small">
                  ...
                  <div>
                    <Switch
                      ...
                      disabled={disabled}
                    />
                    <Text style={{ marginLeft: '8px', fontSize: '12px' }}>Force Expression Mode</Text>
                  </div>
                </Space>
              </Panel>
            </Collapse>
...
const ExpressionHelpContent: React.FC<{ context?: any }> = ({ context }) => (
    ...
    <Collapse>
      <Panel header="Common Expressions" key="common">
        <ul style={{ paddingLeft: '20px' }}>
          <li><code>{`{{ $json.fieldName }}`}</code> - Access input field</li>
          <li><code>{`{{ $json.user.name }}`}</code> - Access nested field</li>
          <li><code>{`{{ $json.items[0].value }}`}</code> - Access array element</li>
          <li><code>{`{{ $json.price * 1.2 }}`}</code> - Mathematical operations</li>
          <li><code>{`{{ $json.status === 'active' ? 'enabled' : 'disabled' }}`}</code> - Conditional logic</li>
        </ul>
      </Panel>
      <Panel header="Built-in Functions" key="functions">
        <ul style={{ paddingLeft: '20px' }}>
          <li><code>{`{{ now() }}`}</code> - Current date and time</li>
          <li><code>{`{{ today() }}`}</code> - Current date (no time)</li>
          <li><code>{`{{ upper($json.name) }}`}</code> - Convert to uppercase</li>
          <li><code>{`{{ lower($json.name) }}`}</code> - Convert to lowercase</li>
          <li><code>{`{{ trim($json.name) }}`}</code> - Remove whitespace</li>
        </ul>
      </Panel>
    </Collapse>
  </Space>
...
const AdvancedAssignmentCollection: React.FC<AdvancedAssignmentCollectionProps> = ({
  ...
  const handleUpdateAssignment = useCallback((id: string, updates: Partial<IAdvancedAssignment>) => {
  ...
  const handleAddAllFields = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(inputData.length, 100); i++) { // Sample first 100 items
      ...
      for (let i = 0; i < fieldArray.length; i++) {
        ...
        for (const item of inputData.slice(0, 10)) { // Check first 10 items
          if (item.json) {
    ...
    <div className="advanced-assignment-collection">
      ...
      <div className="bulk-operations">
        ...
        <Text type="secondary" style={{ fontSize: '12px' }}>
          Configure field assignments with type validation, expression support, and drag-and-drop reordering.
          {enableExpressionEditor && (
            <>
              {' '}Use <Tag size="small">{{ expression }}</Tag> syntax for dynamic values.
            </>
          )}
        </Text>
      </div>
      ...
      {bulkOperationProgress?.visible && (
        <div style={{ marginBottom: '16px' }}>
          <Progress
            percent={Math.round((bulkOperationProgress.current / bulkOperationProgress.total) * 100)}
            status="active"
...
function extractFieldNames(obj: any, prefix: string, fieldNames: Set<string>, maxDepth: number): void {
