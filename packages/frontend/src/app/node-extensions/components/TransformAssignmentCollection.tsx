...
import { Button, Select, Input, Switch, Tooltip, Card, Space, Typography, Divider } from 'antd'
import { PlusOutlined, DeleteOutlined, DragOutlined, ClearOutlined, AppstoreAddOutlined } from '@ant-design/icons'
...
const SortableAssignmentItem: React.FC<{
  ...
  const handleFieldChange = useCallback((field: keyof FieldAssignment, value: any) => {
      ...
      case 'arrayValue':
        return (
          <Input.TextArea
            ...
            value={typeof assignment.value === 'string' ? assignment.value : JSON.stringify(assignment.value || [])}
            onChange={(e) => handleFieldChange('value', e.target.value)}
            rows={2}
            disabled={disabled}
          />
...
const TransformAssignmentCollection: React.FC<TransformAssignmentCollectionProps> = ({
  ...
  // Update assignment
  const handleUpdateAssignment = useCallback((id: string, updates: Partial<FieldAssignment>) => {
