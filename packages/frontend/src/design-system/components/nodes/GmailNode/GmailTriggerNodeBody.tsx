...
const GmailTriggerNodeBody: React.FC<CustomNodeBodyProps> = ({ 
  ...
  const getPollFrequencyDisplay = (pollTimes: any) => {
    ...
    switch (mode) {
      ...
      case 'everyHour': return `Every Hour (${pollTimes.minute || 0}m)`
      case 'everyDay': return `Daily ${pollTimes.hour || 9}:${String(pollTimes.minute || 0).padStart(2, '0')}`
      case 'everyWeek': return `Weekly ${['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][pollTimes.weekday || 1]}`
      case 'everyMonth': return `Monthly (Day ${pollTimes.dayOfMonth || 1})`
      case 'customInterval': return `Every ${pollTimes.intervalMinutes || 5}m`
      case 'customCron': return 'Custom Cron'
