  ...
  loadIntegrations: async () => {
    ...
    try {
      ...
      if (config.features.enableMockData) {
        ...
        const integrationsWithMockConnections = allIntegrations.map(
          (integration: Integration) => {
            // Connect a few popular integrations by default for demo purposes
            if (
              ['slack', 'gmail', 'google-drive', 'github', 'trello'].includes(
                integration.id
              )
            ) {
        ...
        set({
          ...
          connectedIntegrations: integrationsWithMockConnections.filter(
            (i: any) => i.isConnected
          ),
        })
      } else {
        ...
        try {
          const connectionStatuses =
            await integrationService.getIntegrationStatuses()

          const integrationsWithStatus = allIntegrations.map(
            (integration: Integration) => ({
              ...integration,
              isConnected:
                connectionStatuses[integration.id]?.connected || false,
          ...
          set({
            ...
            connectedIntegrations: integrationsWithStatus.filter(
              (i: any) => i.isConnected
            ),
          })
        } catch (error: any) {
          ...
          if (
            ...
            console.warn(
              'Integration status endpoint not available (404), using local data'
            )
          } else {
            console.warn(
              'Failed to load integration statuses, using local data:',
              error
            )
          }
    ...
    try {
      ...
      set(state => {
        const updatedIntegrations = state.integrations.map(i =>
          i.id === id
            ? { ...i, isConnected: true, connectionConfig: integrationConfig }
            : i
        )
  ...
  disconnectIntegration: async id => {
    ...
    try {
      ...
      set(state => {
        const updatedIntegrations = state.integrations.map(i =>
          i.id === id
            ? { ...i, isConnected: false, connectionConfig: undefined }
            : i
        )
