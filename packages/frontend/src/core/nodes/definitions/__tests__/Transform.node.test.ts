...
describe('TransformNode', () => {
  ...
  describe('Node Definition', () => {
    ...
    it('should have conditional properties for different modes', () => {
      const description = transformNode.description;
      const fieldsToAddProperty = description.properties.find((p) => p.name === 'fieldsToAdd');
      const fieldsToRemoveProperty = description.properties.find(
        (p) => p.name === 'fieldsToRemove'
      );

      expect(fieldsToAddProperty?.displayOptions?.show?.mode).toContain('addFields');
      expect(fieldsToRemoveProperty?.displayOptions?.show?.mode).toContain('removeFields');
    });
  });
  ...
  describe('Node Registration', () => {
    it('should be registerable in node registry', () => {
      ...
      expect(nodeRegistry.getNodeTypeDescription('transform')).toEqual(transformNode.description);
    });
      ...
      description.properties
        ...
        .forEach((property) => {
          if (property.displayOptions?.show) {
            Object.entries(property.displayOptions.show).forEach(([key, values]) => {
