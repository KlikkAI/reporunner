...
export class AIAgent implements INodeType {
  description: INodeTypeDescription = {
    ...
    version: 1,
    description:
      'Process data using AI models like OpenAI GPT, Claude, and Gemini',
    defaults: {
      name: 'AI Agent',
      color: '#6366f1',
    },
    ...
    properties: [
      ...
      },
      {
        ...
        required: true,
        description:
          'User prompt/message for the AI. Use {{input}} for dynamic content',
        default: '',
      ...
      },
      {
        ...
        required: false,
        description:
          'Nucleus sampling parameter (0.1 = only top 10% likely tokens)',
        default: 1.0,
        min: 0.1,
        max: 1.0,
        step: 0.1,
      ...
      },
      {
        ...
        required: false,
        description:
          'Increase likelihood of talking about new topics (OpenAI only)',
        default: 0,
        min: -2.0,
        max: 2.0,
        step: 0.1,
      ...
      if (agentType === 'classifier') {
        ...
        result = {
          ...
          metadata: {
            ...
            classification: isCustomerSupport
              ? 'Customer Support'
              : 'Non-Support',
          },
        }
  ...
  private classifyCustomerSupportEmail(emailText: string): boolean {
    if (!emailText) return false

    const text = emailText.toLowerCase()

    // Customer support keywords from workflow.json
    ...
    const hasSupporKeywords = supportKeywords.some(keyword =>
      text.includes(keyword)
    )

    // Non-support indicators
    ...
    const hasNonSupportKeywords = nonSupportKeywords.some(keyword =>
      text.includes(keyword)
    )

    // Classification logic
    if (hasNonSupportKeywords) return false
    if (hasSupporKeywords) return true

    // Default: if uncertain, classify as support to be safe
    return text.includes('?') || text.includes('help') || text.includes('issue')
  }
}
