...
interface ContextResolver {
  id: string
  resolve: (context: WorkflowContext) => Promise<ResolvedContext>
  priority: number
}
...
class NodeRegistry {
  ...
  public async getEnhancedNodeType(
    ...
    const resolvedContext = await this.resolveNodeContext(nodeId, context)
    if (resolvedContext) {
  ...
  private deriveNodeModes(nodeType: EnhancedIntegrationNodeType): ('trigger' | 'action' | 'webhook' | 'poll')[] {
  ...
  private extractResources(nodeType: EnhancedIntegrationNodeType): string[] {
    ...
    if (resources.size === 0) {
      if (nodeType.id && nodeType.id.toLowerCase().includes('gmail')) {
        resources.add('email'), resources.add('label'), resources.add('draft'), resources.add('thread')
      }
    }
  ...
  private async resolveNodeContext(
    ...
    const resolvers = Array.from(this.contextResolvers.values())
