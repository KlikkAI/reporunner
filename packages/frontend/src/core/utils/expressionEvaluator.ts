...
class ExpressionFunctions {
  ...
  static average(arr: number[]): number {
    return Array.isArray(arr) && arr.length > 0 ? this.sum(arr) / arr.length : 0
  }
...
class ExpressionEvaluator {
  ...
  private options: IExpressionOptions
...
class ExpressionUtils {
  ...
  static getExpressionSuggestions(context: IExpressionContext): Array<{
    ...
    suggestions.push(
      { label: 'now()', value: '{{ now() }}', description: 'Current date and time', type: 'function' },
      { label: 'today()', value: '{{ today() }}', description: 'Current date (no time)', type: 'function' },
      { label: 'trim()', value: '{{ trim($json.field) }}', description: 'Remove whitespace', type: 'function' },
      { label: 'upper()', value: '{{ upper($json.field) }}', description: 'Convert to uppercase', type: 'function' },
      { label: 'lower()', value: '{{ lower($json.field) }}', description: 'Convert to lowercase', type: 'function' }
    )
