  ...
  const backendNodes: BackendWorkflowNode[] = nodes.map(node => {
    ...
    return {
      ...
      credentials: nodeData.credentials
        ? mapCredentialsToBackend(nodeData.credentials)
        : undefined,
 ...
 */
function convertGmailParameters(
  config: any,
  nodeType: string
): Record<string, any> {
  if (nodeType === 'trigger') {
  ...
  if (nodeType === 'createDraft') {
    return {
      ...
      options: {
        threadId:
          config.threadId || '={{ $("Edit Fields").item.json.threadid }}',
...
function convertConditionParameters(config: any): Record<string, any> {
  ...
  return {
    rules: {
      values: outputs.map((output: any, index: number) => ({
        conditions: {
          options: {
            caseSensitive: true,
            leftValue: '',
          ...
          conditions: [
            {
              leftValue:
                output.condition ||
                '={{ $json.output.parseJson().customerSupport }}',
              rightValue: '',
              operator: {
                type: 'boolean',
                operation: output.label.toLowerCase().includes('support')
                  ? 'true'
                  : 'false',
                singleValue: true,
              },
 ...
 */
export function exportCustomerSupportMvp(
  nodes: Node[],
  edges: Edge[]
): BackendWorkflow {
...
export function validateWorkflowForExport(
  ...
  const nodesWithoutCredentials = nodes.filter(node => {
    const needsCredentials = [
      ...
      'ollama',
      'pgvector',
    ].includes(node.data.integration)
