...
// Define IExpressionContext locally to avoid circular imports
...
class AdvancedTypeValidator {
  ...
  constructor(options: ITypeConversionOptions = { ignoreConversionErrors: false, strictMode: false }) {
  ...
  private validateConvertedValue(value: any, targetType: string): { valid: boolean; error?: string; warnings?: string[] } {
...
class AssignmentValidator {
  ...
  validateAssignment(assignment: IAssignmentValue, context?: IExpressionContext): ITypeValidationResult {
    // Extract the appropriate value based on type
    const value = this.extractAssignmentValue(assignment)
    
    // Validate and convert
    return this.typeValidator.validate(value, assignment.type, context)
  }
  
  validateAssignments(assignments: IAssignmentValue[], context?: IExpressionContext): {
    valid: boolean
