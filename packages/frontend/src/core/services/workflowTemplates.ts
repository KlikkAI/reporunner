...
export class WorkflowTemplatesService {
  ...
  createWorkflowFromTemplate(
    ...
    const nodes: WorkflowNodeInstance[] = template.nodes.map((templateNode) => {
      ...
      if (nodeData.properties) {
        nodeData.properties = this.substituteVariables(nodeData.properties, variables);
      }
   ...
   */
  getRecommendedTemplates(existingNodes: WorkflowNodeInstance[], limit = 5): WorkflowTemplate[] {
    const nodeTypes = new Set(existingNodes.map((node) => node.type));
    const integrations = new Set(
      existingNodes.map((node: any) => node?.data?.integration).filter(Boolean) as string[]
    );
    ...
    const patterns: AutomationPattern[] = [
      {
        ...
        benefits: ['Simple to understand', 'Predictable execution', 'Easy debugging'],
        complexity: 'simple',
      },
      {
  ...
  private calculateRelevanceScore(
    ...
    const nodeTypeOverlap = Array.from(nodeTypes).filter((type) => templateNodeTypes.has(type));
