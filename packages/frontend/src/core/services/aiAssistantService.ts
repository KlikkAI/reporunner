...
export class AIAssistantService {
  ...
  private calculateWorkflowComplexity(workflow: WorkflowDefinition): number {
    ...
    return Math.min(
      1,
      (nodeCount * 0.3 + edgeCount * 0.2 + conditionalNodes * 0.5) / 10,
    );
  }
  ...
  private generateSuggestions(
    ...
    if (analysis.performance.bottlenecks.length > 0) {
      suggestions.push({
        ...
        reasoning:
          "Performance analysis detected bottlenecks that can be optimized",
        estimatedBenefit: {
    ...
    if (analysis.reliability.missingErrorHandling.length > 0) {
      suggestions.push({
        ...
        suggestedChanges: this.generateErrorHandlingChanges(
  ...
  private generatePerformanceChanges(
    ...
    if (
      performance.optimizationOpportunities.includes(
  ...
  private classifyIntent(text: string): string {
    ...
    if (text.includes("data") || text.includes("process"))
