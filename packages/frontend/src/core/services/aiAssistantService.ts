...
import type { WorkflowDefinition, WorkflowNodeInstance } from '@/core/nodes/types';
...
export class AIAssistantService {
  ...
  private calculateWorkflowComplexity(workflow: WorkflowDefinition): number {
    const nodeCount = workflow.nodes.length;
    const edgeCount = Object.keys(workflow.connections || {}).length;
    const conditionalNodes = workflow.nodes.filter((node: any) => node.type === 'condition').length;
  ...
  private generatePerformanceChanges(
  ...
  private generateErrorHandlingChanges(
  ...
  private generateErrorDiagnoses(_workflow: WorkflowDefinition, patterns: any[]): ErrorDiagnosis[] {
  ...
  private analyzeDataFlow(_workflow: WorkflowDefinition, _previousNodes: string[]): string[] {
