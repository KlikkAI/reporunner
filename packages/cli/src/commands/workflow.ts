...
export const workflowCommands = new Command('workflow').alias('wf').description('Manage workflows');
...
workflowCommands
  ...
  .action(async (workflowId, options) => {
    ...
    try {
      ...
      console.log(`🔗 URL: https://app.reporunner.dev/workflows/${workflowId || 'new'}`);
    } catch (error) {
