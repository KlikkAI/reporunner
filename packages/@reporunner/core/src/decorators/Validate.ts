  ...
  return (target: any, propertyKey: string, descriptor: PropertyDescriptor) => {
    ...
    descriptor.value = function (...args: any[]) {
      ...
      if (typeof result === 'string') {
        throw new Error(
          `Validation failed in ${target.constructor.name}.${propertyKey}: ${result}`
        );
      }
  ...
  return (target: any, propertyKey: string, descriptor: PropertyDescriptor) => {
    ...
    descriptor.value = function (...args: any[]) {
      if (args.length > paramIndex) {
        ...
        if (typeof result === 'string') {
          throw new Error(
            `Parameter validation failed in ${target.constructor.name}.${propertyKey}: ${result}`
          );
        }

        if (!result) {
          throw new Error(
            `Parameter validation failed in ${target.constructor.name}.${propertyKey}`
          );
        }
      }
