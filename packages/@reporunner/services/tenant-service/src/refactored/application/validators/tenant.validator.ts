...
export class TenantValidator extends BaseValidator<Tenant> {
  constructor() {
    ...
    this.addValidation('slug', [
      ...
      this.rules.pattern(/^[a-z0-9-]+$/, 'Tenant slug can only contain lowercase letters, numbers, and hyphens')
    ]);

    this.addValidation('organizationId', [
      this.rules.required('Organization ID is required')
    ]);
