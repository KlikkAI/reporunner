import { UserRole, PermissionType, IUser } from "@reporunner/api-types";
...
export class PermissionEngine {
  ...
  static hasPermission(check: PermissionCheck): boolean {
    ...
    if (resource && resource.ownerId === user.id) {
      return this.checkOwnerPermission(permission);
    }
  ...
  static filterResources<
    T extends { id: string; ownerId?: string; organizationId?: string },
  >(user: IUser, resources: T[], permission: PermissionType): T[] {
  ...
  static getPermissionDiff(
    ...
    const fromPermissions = new Set(this.getRolePermissions(fromRole));
    const toPermissions = new Set(this.getRolePermissions(toRole));
  ...
  static getResourceAccess(
    ...
    return {
      canView: this.hasPermission({ user, permission: permissions[0] }),
      canCreate: this.hasPermission({ user, permission: permissions[1] }),
      canEdit: this.hasPermission({ user, permission: permissions[2] }),
      canDelete: this.hasPermission({ user, permission: permissions[3] }),
      canExecute: this.hasPermission({ user, permission: permissions[4] }),
    };
  }
}
