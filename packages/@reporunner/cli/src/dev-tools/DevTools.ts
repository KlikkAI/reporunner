...
export class DevTools {
  ...
  private profiler?: NodeJS.Timeout;
  ...
  async testWorkflow(workflowId: string, _testData?: unknown): Promise<WorkflowTestResult> {
  ...
  startWatching(): void {
    ...
    for (const pattern of this.config.watchPaths) {
      // Mock watcher implementation - TODO: Implement actual file watching
      const watcher: FileWatcher = {
        close: () => {
          // Watcher cleanup will be implemented when actual file watching is added
        },
      };
  ...
  generateMockData(_schema: unknown): Record<string, unknown> {
  ...
  private getWorkflowTemplate(type: string): WorkflowTemplate {
    const templates = {
      basic: {
        nodes: [
          {
  ...
  private getNodeTemplate(type: string): NodeTemplate {
    const templates = {
      action: {
        type: 'action',
        properties: {
